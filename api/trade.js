 ...
 * 转赠
 */
export function giveOthers(params) {
	return http.request({
		url: `/wallet/wallet/transferAccounts?price=${params.price}&&username=${params.username}&&mobile=${params.mobile}`,
		method: Method.POST,
		needToken: true
	});
}
 ...
 * 创建充值订单
 */
export function createRechargeTrade(params) {
	return http.request({
		url: "/trade/recharge",
		method: Method.POST,
		needToken: true,
 ...
 * 吊起收银台
 */
export function openCounter(params) {
	return http.request({
