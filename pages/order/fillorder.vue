<template>
	<div class="wrapper">
		...
		<div class="box box2" v-for="(item, index) in orderMessage.cartList" :key="index">
          ...
          " :span="3">
						<u-image borderRadius="10rpx" width="200rpx" height="200rpx" :src="val.goodsSku.original" alt />
					</div>
					<div @click="
            navigateTo(
              '/pages/product/goods?id=' +
	...
	export default {
		...
		methods: {
			...
			clickToAddress() {
				...
				this.navigateTo(
					`/subMine/tools/address?from=cart&way=${this.routerVal.way
          ...
          JSON.stringify(this.routerVal.parentOrder)
					...
					API_Trade.createTrade(submit).then((res) => {
						...
						if (res.id) {
							...
							if (this.orderMessage.priceDetailDTO.billPrice == 0) {
								...
								// #ifndef MP-WEIXIN
								this.navigateTo(
									`/pages/cart/payment/payOrder?trade_sn=${res.sn}&&price=${this.orderMessage.priceDetailDTO.goodsPrice ||this.unitPrice}`
								);
								// #endif
							}
						} else {
			...
			async getOrderList() {
				...
				API_Trade.getCheckoutParams(this.routerVal.way).then((res) => {
					 ...
					 */
					this.$store.state.canUseCoupons = res.canUseCoupons;
					this.$store.state.cantUseCoupons = res.cantUseCoupons;

					if (!res.memberAddress) {
						// 获取会员默认地址
						this.getUserAddress();
					} else {
						this.address = res.memberAddress;
						res.memberAddress.consigneeAddressPath =
							res.memberAddress.consigneeAddressPath.split(",");
					}
					if (res.storeAddress) {
					...
					if (
						res.notSupportFreight &&
						res.notSupportFreight.length != 0
					) {
						this.notSupportFreight = res.notSupportFreight;

						res.notSupportFreight.forEach((item) => {
