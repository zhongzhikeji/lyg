<template>
	<div class="wrapper">
		<div v-if="!wechatLogin">
			...
			<view class="" v-if="!is_login">
				<u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="请输入手机号 (11位)"
					class="mobile" focus v-model="regform.mobilePhone" type="number" maxlength="11" />
				<u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="请输入密码"
					class="mobile" v-model="regform.password" type="text" />
				<u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="请输入用户名"
					class="mobile" v-model="regform.username" type="text" />
				<!-- <div :class="!enableFetchCode ? 'disable' : 'fetch'" @click="fetchCode" class="btn">
					获取验证码
				</div> -->
				<div :class="!enableFetchCode ? 'disable' : 'fetch'" @click="registHandle" class="btn">
					注册
				</div>
			</view>
			<view class="" v-if="is_login">
				<u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="请输入手机号 (11位)"
					class="mobile" focus v-model="logform.mobilePhone" type="number" maxlength="11" />
				<u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="请输入密码"
					class="mobile" v-model="logform.password" type="text" />
				<!-- <u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="请输入用户名"
					class="mobile" v-model="regform.username" type="text" /> -->
				<!-- <div :class="!enableFetchCode ? 'disable' : 'fetch'" @click="fetchCode" class="btn">
					获取验证码
				</div> -->
				<div :class="!enableFetchCode ? 'disable' : 'fetch'" @click="loginHandle" class="btn">
					登录
				</div>
			</view>
	...
	export default {
		...
		methods: {
			...
			loginHandle() {
				...
				smsLogin({
					mobile: this.logform.mobilePhone
				}).then(res => {
					// console.log(res)
			...
			registHandle() {
				...
				regHandle(this.regform).then((res) => {
					console.log(res)
				})
				// uni.request({
				// 	url: 'http://18.162.151.103:28006/buyer/passport/member/register',
				// 	method: 'POST',
				// 	data: this.regform,
				// 	header: {
