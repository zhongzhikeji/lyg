      ...
      <div class="box box2" :class="{ invalid: isInvalid(item) }" v-for="(item, index) in cartDetail.cartList"
        ...
        <view class="tab">
          <view class="store-line">
            ...
            <span class="store-name wes store-line-desc" >
      {{item.storeName}}
            </span>
        
          </view>
         <view class="right-col" v-if="item.canReceiveCoupon" @click="isEdit = !isEdit">
           
            <span>{{isEdit?'退出编辑':'编辑'}}</span>
          </view>
        </view>
...
export default {
  ...
  methods: {
    ...
    navigateToGoods(val) {
      ...
      uni.navigateTo({
        url:
          "/subGoods/goodsInfos/goodsInfos?id=" +
        ...
        API_Trade.getCarts()
          .then((result) => {
            uni.stopPullDownRefresh();
              ...
              let checkOuted = true;
			  console.log(this.cartDetail.cartList,556)
