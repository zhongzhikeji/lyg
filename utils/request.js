...
http.interceptors.request.use(
	(config) => {
		...
		console.log(config)
		// if (config.url == 'https://cashier.sandpay.com.cn/gateway/api/order/pay' || config.url ==
		// 	'payment/cashier/pay/SANDPAY/H5') {
		// 	isAllData = true
		// } else if(config.url == 'https://cashier.sandpay.com.cn/gateway/api/order/pay' || config.url ==
		// 	'payment/cashier/pay/WALLET/NATIVE') {
		// 	isAllData = true
		// }
		// let accessToken =
		// 			'eyJhbGciOiJIUzI1NiJ9.eyJ1c2VyQ29udGV4dCI6IntcInVzZXJuYW1lXCI6XCJ0ZW1wXzFYY0hyUlNCaHNzXCIsXCJuaWNrTmFtZVwiOlwi57u_6YePMDAwMVwiLFwiZmFjZVwiOlwibnVsbFwiLFwiaWRcIjpcIjE2NDA3NjA1NjYxMDA5MjIzNjhcIixcImxvbmdUZXJtXCI6dHJ1ZSxcInJvbGVcIjpcIk1FTUJFUlwifSIsInN1YiI6InRlbXBfMVhjSHJSU0Joc3MiLCJleHAiOjE2ODE0NTY0NDF9.5WqIeQ3MULlx-noM_07trdXAP8PcEwxgG2-BbC9feUA'
...
http.interceptors.response.use(
	async (response) => {
			...
			if (response.statusCode === 403) {
				...
				// refreshTokenFn(storage.getRefreshToken()).then(res => {
				// 	console.log(res)
				// })
			}
			if (response.statusCode === 400) {
				...
				// 	refreshTokenFn(oldRefreshToken)
				// 		.then((res) => {
				// 			console.log(requests)
				// 			let {
				// 				accessToken,
